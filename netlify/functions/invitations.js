  ...
  try {
    ...
    if (action === 'list') {
      const rows = await sb(
        ...
        + '&select=token,nom_page,ville,email,canal,created_at,expires_at,email_envoye_at'
    ...
    if (action === 'create') {
      const nom   = (body.nom_page || '').trim();
      const ville = (body.ville || '').trim();
      const email = (body.email || '').trim().toLowerCase();

      if (!nom)   return json(400, { error: 'Nom de la page requis' });
      ...
      const dejaInscrit = await sb(
        'pages?select=slug&limit=1&or=(email_contact.eq.' + encodeURIComponent(email) + ')'
      );
    ...
    if (action === 'resend') {
      const rows = await sb(
        ...
        + '&select=token,nom_page,ville,email,used_at&limit=1'
      );
      ...
      const ok   = await envoyerInvitation({
        email: inv.email, nom: inv.nom_page, ville: inv.ville, lien,
      });
    ...
    // Marque une invitation comme consommée après création du compte.
    // Fait côté serveur (clé service role) pour contourner la RLS :
    // le pharmacien ne peut pas modifier lui-même la table invitations.
    if (action === 'consume') {
      const token = String(body.token || '');
      if (!token) return json(400, { error: 'Token manquant' });
      await sb('invitations?token=eq.' + encodeURIComponent(token), {
        method: 'PATCH',
        headers: { Prefer: 'return=minimal' },
        body: JSON.stringify({
          used_at: new Date().toISOString(),
          slug_cree: body.slug || null,
        }),
      });
      return json(200, { ok: true });
    }
